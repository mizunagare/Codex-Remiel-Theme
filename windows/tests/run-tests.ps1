...
try {
  ...
  if ($initialTheme.Theme.id -cne 'remiel-seraph-system' -or
    $initialTheme.Theme.name -cne '蕾米埃尔｜星芒圣翼' -or
    ...
    throw 'Default Windows theme did not seed the Remiel wallpaper contract.'
  }
  $preseededThemes = @(Get-DreamSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or
    $preseededThemes[0].Id -cne 'remiel-seraph-system' -or
    $preseededThemes[0].Name -cne '蕾米埃尔｜星芒圣翼') {
    throw 'Remiel was not preseeded in the Windows saved-theme menu.'
  }
  ...
  foreach ($requiredPerformanceBehavior in @(
    ...
    'installRemielIcon(nativeIcons[nativeIcons.length - 1], "seraph")'
  )) {
